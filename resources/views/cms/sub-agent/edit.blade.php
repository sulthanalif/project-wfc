                ...
                <form action="{{ route('sub-agent.update', $subAgent) }}" method="post">
                    ...
                    @csrf
                            <div>
                                ...
                                <input id="name" name="name" type="text" class="form-control w-full" placeholder="Masukkan Nama Lengkap Sub Agent" required value="{{ $subAgent->name }}">
                            </div>
                            @hasrole('super_admin')
                            <div class="mt-3">
                                ...
                                <select class="form-select mt-2 sm:mr-2" id="agent_id" name="agent_id" required>
                                    ...
                                    @foreach ($agents as $agent)
                                        <option value="{{ $agent->id }}"  {{ ($subAgent->agent_id == $agent->id) ? 'selected' : '' }}>{{ $agent->agentProfile->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @endhasrole
                            ...
                            <div class="mt-3">
                                ...
                                <input id="phone_number" name="phone_number" type="number" class="form-control w-full" placeholder="Masukkan Nomor Telepon" minlength="11" maxlength="13" required value="{{ $subAgent->phone_number }}">
                            </div>

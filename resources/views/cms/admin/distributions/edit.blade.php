    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12">
            ...
            <div class="intro-y box p-5">
                <form action="{{ route('distribution.update', $distribution) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div>
                        ...
                        <input id="distribution_number" name="distribution_number" type="text" class="form-control w-full"
                            value="{{ $distribution->distribution_number }}" readonly>
                    ...
                    <div>
                        ...
                        <input id="driver" name="driver" type="text" value="{{ $distribution->driver }}" class="form-control w-full"
                         required>
                        @error('date')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                    ...
                    <div>
                        ...
                        <input id="police_number" name="police_number" type="text" value="{{ $distribution->police_number }}" class="form-control w-full"
                         required>
                        @error('date')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                    ...
                    <div class="mt-3">
                        ...
                        <select class="form-select mt-2 sm:mr-2" id="order_id" name="order_id">
                            @foreach ($orders as $order)
                                <option value="{{ $order->id }}" {{ $order->id == $distribution->order_id ? 'selected' : '' }}>{{ $order->order_number }} - {{ $order->agent->agentProfile->name }}</option>
                            @endforeach
                        </select>
                    </div>

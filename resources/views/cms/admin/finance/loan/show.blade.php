@extends('cms.layouts.app', [
    'title' => 'Pembayaran Pinjaman/Piutang',
])

@section('content')
    <div class="intro-y flex items-center mt-8 gap-2">
        <h2 class="text-lg font-medium mr-auto">
            Pembayaran Pinjaman/Piutang #{{ $loan->borrower_name }}
        </h2>

        <a class="btn btn-primary btn-sm py-2 px-3 shadow-md flex items-center ml-2" href="javascript:;" data-tw-toggle="modal"
            data-tw-target="#payment-confirmation-modal">
            Setor </a>
        @include('cms.admin.finance.loan.modal.payment', ['loan' => $loan])

        <a href="{{ route('loan.index') }}" class="btn px-2 box"><i data-lucide="arrow-left" class="w-4 h-4"></i></a>
    </div>
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2">
                ...
                <tbody>
                    ...
                    @else
                        @foreach ($loan->loanPayments->sortByDesc('date') as $payment)
                            <tr class="intro-x">
                                <td>
                                        ...
                                        <div class="w-10 h-10 image-fit zoom-in">
                                            ...
                                            @else
                                                <img alt="PAKET SMART WFC" class="tooltip rounded-full"
                                                    ...
                                                    src="{{ route('getImage', ['path' => 'loan_payments', 'imageName' => $payment->photo]) }}"
                ...
                @if ($loan->loanPayments->first())
                    <tfoot>
                        <tr>
                            <th colspan="2" class="text-center">SISA PEMBAYARAN</th>
                            <th class="text-center">
                                Rp.
                                {{ number_format($loan->loanPayments->sortByDesc('created_at')->first() ? $loan->amount - $loan->loanPayments()->sum('pay') : $loan->amount, 0, ',', '.') }}
                            </th>
                        </tr>
                    </tfoot>
                @endif

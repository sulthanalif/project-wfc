            ...
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-2xl font-bold leading-8 mt-6">Rp.
                        {{ number_format($totalLoans, 0, ',', '.') }}</div>
                    <div class="text-base text-slate-500 mt-1">Total Pinjaman/Piutang</div>
                </div>
            </div>
        </div>
        <div class="col-span-12 sm:col-span-6 xl:col-span-3 intro-y">
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-2xl font-bold leading-8 mt-6">Rp.
                        {{ number_format($totalPayments, 0, ',', '.') }}</div>
                    <div class="text-base text-slate-500 mt-1">Total Dibayar</div>
                </div>
            </div>
        </div>
        <div class="col-span-12 sm:col-span-6 xl:col-span-3 intro-y">
            <div class="report-box zoom-in">
                <div class="box p-5">
                    ...
                    <div class="text-base text-slate-500 mt-1">Sisa Pinjaman/Piutang</div>
                </div>
            </div>
        </div>
    </div>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            ...
            <div class="dropdown mr-2">
                ...
                <div class="dropdown-menu w-40">
                    <ul class="dropdown-content">
                        <li>
                            <a href="{{ route('loan.export') }}" class="dropdown-item"> <i data-lucide="download"
            ...
            @if ($loans instanceof \Illuminate\Pagination\LengthAwarePaginator)
                <div class="hidden md:block mx-auto text-slate-500">Menampilkan {{ $loans->firstItem() }} hingga
                    {{ $loans->lastItem() }} dari {{ $loans->total() }} data</div>
            @else
                <div class="hidden md:block mx-auto text-slate-500">Menampilkan semua {{ $loans->count() }} data
                </div>
            @endif
                    ...
                    @else
                        @foreach ($loans as $loan)
                            <tr class="intro-x">
                                ...
                                <td>
                                    <span
                                        class="text-slate-500 flex items-center justify-center">{{ \Carbon\Carbon::parse($loan->date)->format('d-m-Y') }}</span>
                                </td>
                                <td>
                                    <span class="text-slate-500">{!! $loan->description !!}</span>
                                </td>
                            ...
                            <div id="delete-confirmation-modal{{ $loan->id }}" class="modal" tabindex="-1"
                                aria-hidden="true">
        ...
        @if ($loans instanceof \Illuminate\Pagination\LengthAwarePaginator)
            <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center">
                {{ $loans->links('cms.layouts.paginate') }}
            </div>
        @endif
        <!-- END: Pagination -->
    </div>
